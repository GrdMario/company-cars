namespace Company.Cars.Presentation.Api.Controllers
{
    using AutoMapper;
    using MediatR;
    ...
    public abstract class ApiControllerBase : ControllerBase
    {
        ...
        protected async Task<IActionResult> ProcessAsync<TCommand, TResponse>(TCommand command)
            where TCommand : IRequest<TResponse>
        {
            TResponse result = await _mediator.Send(command);
        ...
        protected async Task<IActionResult> ProcessAsync<TCommand>(TCommand command)
            where TCommand : IRequest
        {
            await _mediator.Send(command);
